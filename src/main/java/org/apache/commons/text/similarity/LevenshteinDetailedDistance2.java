 ...
 *
 */
public class LevenshteinDetailedDistance2 implements EditDistance<LevenshteinResults> {

    /**
     * Default instance.
     */
    private static final LevenshteinDetailedDistance2 DEFAULT_INSTANCE = new LevenshteinDetailedDistance2();
     ...
     * </p>
     *
     * @see LevenshteinDetailedDistance2#getDefaultInstance()
     */
    public LevenshteinDetailedDistance2() {
        this(null);
    }
     ...
     */
    public LevenshteinDetailedDistance2(final Integer threshold) {
     ...
     */
    public static LevenshteinDetailedDistance2 getDefaultInstance() {
    ...
    private static LevenshteinResults limitedCompare(CharSequence left, CharSequence right, int threshold) {
        ...
        // of the other
        if (n == 0) {
            return m <= threshold ? new LevenshteinResults(m, 0, 0, 0) : new LevenshteinResults(-1, 0, 0, 0);
        } else if (m == 0) {
            return n <= threshold ? new LevenshteinResults(n, 0, 0, 0) : new LevenshteinResults(-1, 0, 0, 0);
        }
        ...
        // distance
        if (p[n] <= threshold) {
            return new LevenshteinResults(p[n], 0, 0, 0);
        }
        return new LevenshteinResults(-1, 0, 0, 0);
    }
    ...
    private static LevenshteinResults unlimitedCompare(CharSequence left, CharSequence right) {
        ...
        if (n == 0) {
            return new LevenshteinResults(m, 0, 0, 0);
        } else if (m == 0) {
            return new LevenshteinResults(n, 0, 0, 0);
        }
        ...
        for (j = 1; j <= m; j++) {
            ...
            for (i = 1; i <= n; i++) {
                ...
                if (d[i] < smallestValue) {
                    smallestValue = d[i];
                    smallestValueCost = cost;
                }
            }
            if (smallestValue > previousSmallestValue && j < i) {
                deleteCount++;
            }
            previousSmallestValue = smallestValue;
            System.out.println(String.format("%d, %d", smallestValue, smallestValueCost));
            // copy current distance counts to 'previous row' distance counts
            tempD = p;
            p = d;
            d = tempD;
        ...
        return new LevenshteinResults(p[n], insertCount, deleteCount, substituteCount);
    }
